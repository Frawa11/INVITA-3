...
import { Calendar, Music, Image as ImageIcon, User, MapPin, Phone, MessageCircle, Plus, Trash2, Utensils, Wine, PartyPopper, Camera, Users, Star, Palette, Type, Layout, Video, FileText, Zap, Shirt, Gift } from "lucide-react";
...
export interface EventData {
    ...
    heroImageUrl?: string;
    heroImageUrl?: string;
    gallery: string[]; // Changed from galleryUrl1/2 to array
    musicUrl: string;
    musicUrl: string;
    ...
    gifts?: string; // Nuevo campo Regalos
            ...
            <div className="space-y-6 pb-10">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    {formData.layoutTemplate === 'video' && (
                        <div className="space-y-2">
                            <label className="text-sm font-medium text-gray-700 flex items-center gap-1">
                                <Video className="w-4 h-4 text-purple-600" /> URL de Video (mp4)
                            </label>
                            <input
                                type="url"
                                name="videoUrl"
                ...
                <div className="col-span-1 md:col-span-2 space-y-3">
                    <label className="text-sm font-medium text-gray-700 flex items-center justify-between">
                        <span>Galería de Fotos (Máx. 15)</span>
                        <button
                            onClick={() => setFormData(prev => ({ ...prev, gallery: [...(prev.gallery || []), ""] }))}
                            disabled={(formData.gallery?.length || 0) >= 15}
                    ...
                    <div className="grid grid-cols-1 gap-3">
                        ...
                        {(!formData.gallery || formData.gallery.length === 0) && (
                            <p className="text-xs text-gray-400 italic">No hay fotos en la galería.</p>
                        )}
                    </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-2">
                        <label className="text-sm font-medium text-gray-700 flex items-center gap-1">
                            <Gift className="w-4 h-4 text-purple-600" /> Información de Regalos (Opcional)
                        </label>
                        <input
                            type="text"
                            name="gifts"
                            ...
                            onChange={handleChange}
                            placeholder="Ej: Lluvia de Sobres, Lista de Novios..."
                            className={inputClass}
                        />
                    </div>
                </div>
